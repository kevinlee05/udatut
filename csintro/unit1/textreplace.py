...
def findandreplace(line, marker, replacement):
    endbefore = line.find(marker)
    startafter = endbefore + len(marker)
    before = line[0:endbefore]
    after = line[startafter:]
...
class Test(unittest.TestCase):
    def test(self):
        self.assertEqual(findandreplace("I will now go to sleep and be AFK until lunch time tomorrow.", "AFK", "away from keyboard"), 'I will now go to sleep and be away from keyboard until lunch time tomorrow.')
        self.assertEqual(findandreplace("The eruption of the volcano EY in 2010 disrupted air travel in Europe for 6 days.", "EY", "Eyjafjallajokull"), 'The eruption of the volcano Eyjafjallajokull in 2010 disrupted air travel in Europe for 6 days.')
